...
public class Robot extends IterativeRobot {
  ...
  public void intakeLift() { // This is the function for the lift mechanism for the intake
    ...
    if(sticky.getRawButton(button) && encode && thing == 0){
      thing = 1;
    }else if(!sticky.getRawButton(button) && encode && thing == 1){
      thing = 2;
    }else if(sticky.getRawButton(button) && encode && thing == 2){
      thing = 3;
    }else{
      thing = 0;
    }
    if(thing == 1 || thing == 2){
      inLift.set( Speed(intakePosition, ball, 84, 1) );
    }
