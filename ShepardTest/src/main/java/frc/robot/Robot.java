...
public class Robot extends IterativeRobot {
  ...
  public void smartDashboard() {
    ...
    SmartDashboard.putNumber("Jack Current", Jack.getOutputCurrent());
  ...
  // // Or multiplied by a constant, or adding a constant to make up for the
  // robot's weight
  // }
  // else {
  // backLift.setMotor(0);
  ...
  public void LIDAR() {
    ...
    if (!message.isBlank() && !message.isEmpty() && !dist) {
  ...
  public void logictechController() {
    ...
    /*
     * if(logic.getRawButton(lb) && !x52HatchOverride){ Hatch.set(0.5);//closed
     * }else if(logic.getRawButton(rb) && !x52HatchOverride){ Hatch.set(-1);//open }
     */

    if (logic.getRawButton(y) && !upLimitIntake && !x52InLiftOverride) {
      inLift.set(1);// up
    } else if (logic.getRawButton(b) && !lowLimitIntake && !x52InLiftOverride) {
      inLift.set(-1);// down
    }
  }
  ...
  public void ball() { // this is the fuction for the ball shooter on the elevator
    if (sticky.getRawButton(Fire)) {
      outBall.set(ControlMode.PercentOutput, 1);// spin out
    } else if (sticky.getRawButton(D)) {
      outBall.set(ControlMode.PercentOutput, -0.5);// spin in
    } else {
  ...
  public void jack() { // this is the fuction for that thing on the back of the robot
    ...
    if (sticky.getRawButton(E)) {
      // power the Jack device down
      Jack.set(ControlMode.PercentOutput, 0.75);
      x52JackOverride = true;
    } else if (sticky.getRawButton(i)) {
      // power the Jack device up
      Jack.set(ControlMode.PercentOutput, -0.75);
      x52JackOverride = true;
    } else {
      Jack.set(ControlMode.PercentOutput, 0.0);
  ...
  public double simpleSpeed(double currentPos, double desiredPos, double length) {
    double simple;
    if (currentPos < desiredPos - length) {
      simple = 1;
    } else if (currentPos > desiredPos + length) {
      simple = -1;
    } else {
      simple = 1 - (1 - ((desiredPos - currentPos) / length));
    }
  ...
  public double Speed(double current, double desired, double gap, double max) {
    /*
     * variable definitions: current; the changing value for the function, like x
     * desired; set value that we want to move to gap; how long you want to have a
     * slow down, might have a relation to distance
     */
